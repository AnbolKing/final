export const LIST_CARDS = [
  {
    imgUrl: 'https://img0.baidu.com/it/u=1431156611,678371665&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
    desc: '沃柑2斤小果产地直达，川哥哥的水果你确定不要嘛；沃柑2斤小果产地直达，川哥哥的水果你确定不要嘛',
    tags: ['皮薄多汁', '果肉细腻'],
    price: '9.9',
    id: '0bggut',
  },
  {
    imgUrl: 'https://img2.baidu.com/it/u=399756892,3950743515&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
    desc: '道具仿真书摆设简约样板房设计书柜展示排列假书',
    tags: ['全新正品', '限时特惠'],
    price: '9.9',
    id: '4567uhb',
  },
  {
    imgUrl: 'https://img1.baidu.com/it/u=703931155,2206972148&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
    desc: '合肥婴儿纸巾什么品牌好绿色环保',
    tags: ['天然柔滑'],
    price: '9.9',
    id: '876tghj',
  },
  {
    imgUrl: 'https://img2.baidu.com/it/u=3766730286,2066554033&fm=253&fmt=auto&app=138&f=JPEG?w=300&h=300',
    desc: '耐克空军一号AF1棕色男生潮流男女百搭男鞋女鞋',
    tags: ['正品包退', '货到付款'],
    price: '589.9',
    id: '45678uygvbnj',
  },
  {
    imgUrl: 'https://img2.baidu.com/it/u=3094828638,1112296414&fm=26&fmt=auto',
    desc: '男生束脚裤韩流百搭潮流男生休闲裤黑色裤子长裤',
    tags: ['达人推荐', '送运费险'],
    price: '39.9',
    id: '5678uyhghj',
  },
  {
    imgUrl: 'https://img0.baidu.com/it/u=4087986800,2849193112&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=500',
    desc: '营养鲜享飞鹤星飞帆a2奶粉1段婴幼儿配方牛奶粉一段708',
    tags: ['欧洲进口', '品质保障'],
    price: '199.9',
    id: '34rdfgh',
  },
  {
    imgUrl: 'https://img0.baidu.com/it/u=1431156611,678371665&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
    desc: '沃柑2斤小果产地直达，川哥哥的水果你确定不要嘛；沃柑2斤小果产地直达，川哥哥的水果你确定不要嘛',
    tags: ['皮薄多汁', '果肉细腻'],
    price: '9.9',
    id: '1234rfghj',
  }
];

export const ORDER_ITEM  = [
  {
    store_img: 'https://img2.baidu.com/it/u=2363838054,3930871158&fm=253&fmt=auto&app=138&f=JPEG?w=652&h=500',
    store_name: '上海荣誉体育运动专卖店',
    status: 3,
    good_img: 'https://img1.baidu.com/it/u=883504995,1605864769&fm=26&fmt=auto',
    good_desc: 'Nike耐克 AJ Air Jordan 1 AJ灰绿色',
    good_price: '1499',
    good_nums: 3,
    good_info: '40',
    good_benefits: [1, 2],
    good_all_price: '1499',
    good_need_pay: '1499',
  },
  {
    store_img: 'https://img2.baidu.com/it/u=2363838054,3930871158&fm=253&fmt=auto&app=138&f=JPEG?w=652&h=500',
    store_name: '上海荣誉体育运动专卖店',
    status: 3,
    good_img: 'https://img1.baidu.com/it/u=883504995,1605864769&fm=26&fmt=auto',
    good_desc: 'Nike耐克 AJ Air Jordan 1 AJ灰绿色',
    good_price: '1499',
    good_nums: 3,
    good_info: '40',
    good_benefits: [1, 2],
    good_all_price: '1499',
    good_need_pay: '1499',
  },
  {
    store_img: 'https://img2.baidu.com/it/u=2363838054,3930871158&fm=253&fmt=auto&app=138&f=JPEG?w=652&h=500',
    store_name: '上海荣誉体育运动专卖店',
    status: 3,
    good_img: 'https://img1.baidu.com/it/u=883504995,1605864769&fm=26&fmt=auto',
    good_desc: 'Nike耐克 AJ Air Jordan 1 AJ灰绿色',
    good_price: '1499',
    good_nums: 3,
    good_info: '40',
    good_benefits: [1, 2],
    good_all_price: '1499',
    good_need_pay: '1499',
  },
  {
    store_img: 'https://img2.baidu.com/it/u=2363838054,3930871158&fm=253&fmt=auto&app=138&f=JPEG?w=652&h=500',
    store_name: '上海荣誉体育运动专卖店',
    status: 3,
    good_img: 'https://img1.baidu.com/it/u=883504995,1605864769&fm=26&fmt=auto',
    good_desc: 'Nike耐克 AJ Air Jordan 1 AJ灰绿色',
    good_price: '1499',
    good_nums: 3,
    good_info: '40',
    good_benefits: [1, 2],
    good_all_price: '1499',
    good_need_pay: '1499',
  }
]

export const MINE_INFO = {
  avatar: 'https://img2.baidu.com/it/u=1652427738,3139747163&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
  name: '勇敢的赫奇',
}

export const ADDRESS = [
  {
    id: '10001',
    name: '王子安',
    phone: '18370605879',
    one_level: '江西省',
    two_level: '吉安市',
    three_level: '永丰县',
    detail: '恩江镇地王大厦5单元0503室',
    isChecked: true,
  },
  {
    id: '10002',
    name: '王大炮',
    phone: '18370605879',
    one_level: '北京',
    two_level: '北京市',
    three_level: '海淀区',
    detail: '上帝街道上地东路1号鹏寰国际大厦大厦6FA区',
    isChecked: false,
  }
]

export const DETAIL = {
  id: '1001',
  swiper_imgs: [
    'https://img1.baidu.com/it/u=2193335351,6115624&fm=253&fmt=auto&app=120&f=JPEG?w=899&h=500',
    'https://img0.baidu.com/it/u=1242512222,1241183799&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=312',
    'https://img0.baidu.com/it/u=1538438019,2605145229&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=281',
  ],
  price: '69.9',
  nums: '535',
  title: '【强者法则全10册】智慧谋略社交心理攻心术谋略人性',
  tags: ['全新正品', '限时特惠'],
  insure: ['假一赔三', '退货补运费', '七天无理由退换货'],
  go: '预计72小时内发货',
  info: {
    产地: '江西',
    生产日期: '',
  },
  store_avatar: 'https://img2.baidu.com/it/u=1652427738,3139747163&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
  name: '易果云官方旗舰店',
  desc: '专注于生态农业发展，为大家种植和甄选大家放心的水果',
  detail_imgs: [
    'https://img0.baidu.com/it/u=1571014671,1329084906&fm=253&fmt=auto&app=138&f=JPEG?w=750&h=500',
    'https://img0.baidu.com/it/u=3310630758,1137644732&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=342',
    'https://img2.baidu.com/it/u=2607105925,3961406987&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=250',
    'https://img1.baidu.com/it/u=1374076241,3989169198&fm=253&fmt=auto&app=138&f=JPEG?w=800&h=500',
  ],
  commodity_label: '规格',
  commodity_img: 'https://img2.baidu.com/it/u=1609425339,3574451547&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500',
  commodity_range: [
    {
      price: '9.9',
      content: '小果2斤装【65mm以下】',
    },
    {
      price: '19.9',
      content: '大果5斤装【85mm以下】',
    },
  ]
}